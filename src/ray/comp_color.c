...
t_rgb	comp_color(const t_scene *scene, const t_hit *hit)
{
	...
	offset = vec_add(hit->p, vec_scal_mul(hit->surf_n, (EPSILON * fmax(1.0,
						vec_len(hit->p)))));
	l_vec = vec_sub(scene->light.cors, offset);
	lv_dir = normalize(l_vec);
	amb_diff[0] = vec_mul(hit->rgb, vec_scal_mul(scene->amb.rgb,
				scene->amb.ratio));
	ndotl = vec_dot(hit->surf_n, lv_dir);
	if (ndotl <= 0)
		return (vec_to_rgb_cl(amb_diff[0]));
	if (light_ray(scene->objs, hit->obj, (t_ray){offset, lv_dir},
		vec_len(l_vec)))
	{
		amb_diff[1] = vec_scal_mul(hit->rgb, scene->light.brightness * fmax(0,
					ndotl));
		return (vec_to_rgb_cl(vec_add(amb_diff[0], amb_diff[1])));
	}
	return (vec_to_rgb_cl(amb_diff[0]));
}
